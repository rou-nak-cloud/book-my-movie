...
import { ChartLineIcon, CircleDollarSignIcon, IndianRupee, PlayCircleIcon, StarIcon, UsersIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  return !loading ? (
    <>
      <Title text1='Welcome, Bakshi TO' text2='Dashboard' />

      <div className='relative flex flex-wrap gap-4 mt-6'>
        <BlurCircle top='-100px' left='0' />
        <div className='flex flex-wrap gap-4 w-full'>
          {dashboardCards.map((card,idx) => (
            <div key={idx} className='flex items-center justify-between px-4 py-3 bg-primary/20 border border-primary-dull/50 rounded-md max-w-65 w-full'>
              <div className=''>
                <h1 className='text-sm'>{card.title}</h1>
                <p className='text-xl font-medium mt-1'>{card.value}</p>
              </div>
              <card.icon className='w-6 h-6 text-pink-300' />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className='relative flex items-center justify-center flex-wrap gap-6 mt-4 max-w-5xl'>
