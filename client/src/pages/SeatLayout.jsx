  ...
  const getShow = async () => {
    ...
    const handleSeatClick = (seatId) => {
      ...
      if(!selectedSeats.includes(seatId) && selectedSeats.length > 4){
        return toast("You can only select only 5 seats")
      }
        ...
        <div className='flex flex-wrap items-center justify-center gap-2'>
          {Array.from({ length: count }, (_,i) => {
            ...
            return(
              ...
              className={`h-8 w-8 rounded border border-primary/60 cursor-pointer hover:text-primary/90 transition-colors ease-in-out 
              ${selectedSeats.includes(seatId) && "bg-primary text-white transition-all ease-in"} 
    ...
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-40'>
      ...
      <p className='text-lg font-semibold px-6'>Available Timings</p>
        <div className='mt-5 space-y-1'>
          ...
          <div key={item.time} onClick={()=>setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-amx rounded-r-md cursor-pointer transition-all ${selectedTime ?.time === item.time ? "bg-primary text-white" : "hover:bg-primary/30"}`}>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        <img src={assets.screenImage} alt="screen layer" className='w-[70%]' />
        ...
        <div className='grid grid-cols-2 gap-12 text-gray-300 text-xs'>
